...
trades <- read_csv("turtle_trades.csv")

# Format for display
trades_display <- trades %>%
  mutate(
    entry_date = as.character(entry_date),
    exit_date = as.character(exit_date),
    entry_price = sprintf("$%.2f", entry_price),
    exit_price = sprintf("$%.2f", exit_price),
    pnl_total = sprintf("$%.2f", pnl_total),
    pnl_pct = sprintf("%.2f%%", pnl_pct),
    stop_loss_price = sprintf("$%.2f", stop_loss_price)
  ) %>%
  select(entry_date, exit_date, entry_price, exit_price, shares, pnl_total, pnl_pct, hold_days, exit_reason)

kable(trades_display, format = "html", caption = "Complete Trade Record (Jan 2025 - Apr 2026)")
```

<div class="highlight-box">
**Download**: A complete CSV file with all trades is available [here](turtle_trades.csv) for your own analysis.
</div>
...
kable(comparison, format = "html", caption = "Strategy Comparison: Turtle vs. Original Breakout")
```
